"""
Written by: Robert Selack
E-mail: robfu@zedat.fu-berlin.de
...
def redundancy_check(conn, table, issue_or_repo_id, title_or_name, relevance, keywords):
    """
    this function is used for caching as it checks inside the database for the given issue or repository and
    extracts the necessary metainformation to display instead of wasting valuable rate-limit requests
    :param conn: connection object to access the database
    :param table: name of the table which will be either 'issue' or 'repository'
    :param issue_or_repo_id: id of the issue or repo to look up inside the database
    :param title_or_name: title or name of the issue or repository to display for the user
    :param relevance: used to check if the current relevance is lower than the one stored inside the database
    :param keywords: keywords to used for another check up
    :return: boolean whether the repository or issue are already stored or not
    """
    try:
        with conn:
            cursor = conn.cursor()
...
def keywords_check(conn, issue_id, keywords):
    """
    function to update the keywords column inside the database if the results was found with additional keywords
    :param conn: connection object to access the database
    :param issue_id: id of the issue
    :param keywords: keywords used for this very instance of iteration to find the result
    :return: returns an KeywordsValues object for redundancy_check() to display what new keywords were added if so
    """
    try:
        with conn:
            cursor = conn.cursor()
...
def get_languages(repo):
    """
    function to extract the programming languages used inside this repository
    :param repo: repository object
    :return: the string of languages to insert into the database
    """
    try:
        languages_str = ''
...
def get_linked_issues(repo, issue):
    """
    TODO this is a function that has a concept of extracting linked issues inside issues but is not consistent due to
    TODO linked issues being possible to be embedded inside text
...
def issue_print(conn, issue_id):
    """
    function that handles the way the results are shown to the user
    :param conn: connection object to access the database
    :param issue_id: id of the issue that is also used to determine repository
    :return: nothing
    """
    try:
        with conn:
...
def page_iterator(auth, keywords, issue_comments, repo_contributors, google_api_key, google_cse_id, path_db):
    """
    the main function that handles the iterative extraction of metainformation for each result
    :param auth: the authentication object of the github API used to make requests
    :param keywords: keywords currently used as they are written inside the configuration file
    :param issue_comments: value of the amount of comments as it is written inside the configuration file
    :param repo_contributors: value of the amount of contributors as it is written inside the configuration file
    :param google_api_key: the api key used for the google search json API taken from the configuration file
    :param google_cse_id: the id used for the google custom search engine taken from the configuration file
    :param path_db: path of the database to create if it does not exist yet which is taken from the configuration file
    :return: nothing
    """
    try:
        relevance = 0
        ...
        for offset in range(0, 100, 10):
            ...
            for res in res_page['items']:
                ...
                try:
                    ...
                    print('Exception inside page_iterator() on line {}:'.format(sys.exc_info()[-1].tb_lineno), repr(e))
                    continue
...
def input_handler(init):
    """
    this function realizes the interactive mode for the user to communicate with the program
    :param init: the initialization object that contains all necessary arguments
    :return: nothing as this handler will be called by itself
    """
    conn = create_connection(init.config.path_of_database)
    print('-----------------------------------------------------------------------------------------------')
    print('sq\t'.rjust(35), '(search query) - to use the query created by the config '
                            'and give out each result for further inspection and insert them into the database')
    print('sn<tab><issue_id><tab><message>\t'.rjust(35), '(set notes) - to set notes for a certain issue')
    print('ss<tab><issue_id><tab><score>\t'.rjust(35), '(set score) - to set the score for a certain issue')
    print('giws<tab><operator><tab><score>\t'.rjust(35), '(get issues where score) - '
                                                         'to get all issues stored in the database '
                                                         'where the score fulfills the condition')
    print('giwm\t'.rjust(35), '(get issues where metrics) - '
                              'to get all issues stored in the database where the metainformation fulfill '
                              'the metrics set inside the configfile')
    print('giwn<tab>note\t'.rjust(35), '(get issues where notes) - '
                                       'to get all issues stored in the database where the set notes inside the issues '
                                       'contain the note')
    print('dr<tab>repo_id\t'.rjust(35), "(download repository) - "
                                        "to download the repository's files into a separate folder")
...
def set_notes_to_issue(issue_id, message, conn):
    ...
    takes a string written by the user and inserts it into the respective issue
    :param issue_id: id for the issue to be updated with
    :param message: the actual note that will be updated inside the issue
    :param conn: connection object to access the database
    :return: nothing
    """
    try:
...
def set_score_of_issue(issue_id, score, conn):
    """
    function for the interactive mode.
    takes a value written by the user and inserts into the respective issue
    :param issue_id: id for the issue to be updated with
    :param score: integer that will be updated inside the issue
    :param conn: connection object to access the database
    :return: nothing
    """
    try:
...
def get_issues_where_score(conn, operator, score):
    """
    function for the interactive mode.
    prints out all the issues of the database where their score inside the database
    is greater, lower than or equal to, depending on the operator set by the user,
    the score set by calling this function
    :param conn: connection object to access the database
    :param operator: <, <=, >, >= or = (all operator accepted by the SQL)
    :param score: the score that is used as a reference for determining which issues to select and print
    :return: nothing
    """
    try:
        with conn:
...
def get_issues_where_notes(conn, notes):
    """
    function for the interactive mode.
    prints out all the issues of the database where their note contains the note set by the user.
    :param conn: depending on the operator set by the user
    :param notes: note to be contained inside the issue.
                    % operators can also be used. Just as everything accepted by the SQL
    :return: nothing
    """
    try:
        with conn:
...
def get_issues_where_metrics(conn, keywords, issue_comments, repo_contributors):
    """
    function for the interactive mode.
    functions the same as the actual google search mode but only using the database as the dataset.
    metrics filtering is also included and the results are sorted by their score, going from highest to lowest.
    :param conn: depending on the operator set by the user
    :param keywords: keywords currently used as they are written inside the configuration file
    :param issue_comments: value of the amount of comments as it is written inside the configuration file
    :param repo_contributors: value of the amount of contributors as it is written inside the configuration file
